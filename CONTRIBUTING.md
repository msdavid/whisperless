...
```bash
git clone <your-fork> && cd hypr-input
...
python3 -c "import ast; ast.parse(open('voice-input').read().split('\n',1)[1])"
~/.local/bin/voice-input --test some-16k-mono.wav     # end-to-end pipeline
```
...
|------|-----------|---------|
| `voice-input` | client: toggle, capture, WS stream, typing | source of truth is the copy here; `install.sh` rewrites the shebang when installing |
...
| `hypr-input.conf` | documented default config | installed config lives at `~/.config/hypr-input/config.conf`; installer merges new keys into existing configs |
| `r2t2.service` | systemd unit **template** (`@PLACEHOLDER@s`) | rendered by install.sh from config |
...
2. **Config is the interface.** New behavior gets a `KEY=value` option with a
   default in `hypr-input.conf`, not a new flag or hardcoded constant. The
...
- `~/.cache/voice-input.log` tail (client side — errors log full tracebacks)
- your `~/.config/hypr-input/config.conf` (no secrets in it)
